...
internal static class SVGLoader
{
    ...
    {
        var stream = typeof(Impasto).Assembly.GetManifestResourceStream(resourcePath);
        if (stream is null) throw new ArgumentException($"{resourcePath} not found in {typeof(Impasto).Assembly.FullName}");
        return stream;
    }
